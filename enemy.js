...
// Stores values related to the different enemies
// Each Enemy is derived from a base Enemy class
// Enemies are assigned their images (sprite sheets) and image dimensions in order to animate
// Main.js continously iterates through an array of enemies in order to update their coordinates
// as well as update the displayed portion of their sprite sheet in order to animate
class Enemy {
    constructor(){
        this.frameX = 0;
        this.frameY = 0;
...
export class WasabiEnemy extends Enemy {
    ...
    update(deltaTime){
        if (deltaTime % this.deltaMultiplier === 1){
            this.game.particles.unshift(new WasabiFire(this.game, this.x + this.width * 0.5, this.y + this.height));
        }
...
export class ChopsticksEnemy extends Enemy {
    constructor(game){
        super();
        this.game = game;
